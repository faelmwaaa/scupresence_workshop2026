...
const props = defineProps({ user: Object, organizations: Array });
...
const eventForm = useForm({
    ...
    title: '', event_date: '', start_time: '', end_time: '', location: '', description: '',
    is_recurring: false, repeat_until: '',
});
...
const fetchSchedules = async (orgId) => {
    ...
    try {
        const res = await fetch(`/pelatih/schedules/${orgId}`, { headers: { 'Accept': 'application/json' } });
...
const submitEvent = () => {
    eventForm.post(route('pelatih.schedules.store'), {
...
const toggleOpen = async (schedule) => {
    ...
    try {
        const res = await fetch(route('pelatih.schedules.toggleOpen', schedule.id), {
            method: 'POST',
...
const openValidation = async (schedule) => {
    ...
    try {
        const res = await fetch(route('pelatih.presences.index', schedule.id), { headers: { 'Accept': 'application/json' } });
        const data = await res.json();
...
const setPresenceStatus = async (presence, status) => {
    ...
    try {
        ...
        if (res.ok) { presence.status = status; }
...
<template>
    <Head title="Home - Pelatih UKM" />
    <div class="page-container bg-gray-50 pb-24">
        <div class="bg-white px-4 pt-12 pb-6 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <p class="text-xs text-gray-500">Pelatih UKM</p>
                    <h1 class="text-xl font-bold text-gray-900">{{ user.name }}</h1>
                </div>
                <img :src="user.profile_picture || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user.name) + '&background=5B2163&color=fff'"
                    class="w-10 h-10 rounded-full object-cover border-2 border-purple-100" />
            </div>
            <!-- Unit Tabs -->
            <div class="mt-2" v-if="organizations.length > 0">
                <UnitDropdown :organizations="organizations" v-model="selectedOrgId" @change="fetchSchedules($event)" />
            </div>
        </div>

        <div v-if="isPending" class="mx-4 mt-6 bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center">
            <div class="text-4xl mb-3">⏳</div>
            <h3 class="font-bold text-amber-800 mb-1">Menunggu Verifikasi Admin</h3>
            <p class="text-sm text-amber-600">Akun kamu sedang diverifikasi oleh Super Admin.</p>
        </div>

        <div v-else class="px-4 mt-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="section-title mb-0">📅 Jadwal Mendatang</h2>
                <button @click="showAddEvent = true" class="flex items-center gap-1.5 bg-brand-purple text-white text-sm font-medium px-4 py-2 rounded-xl shadow-sm hover:bg-brand-purple-dark transition-all active:scale-95">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    Tambah Event
                </button>
            </div>
            <div v-if="schedules.length === 0" class="text-center py-12 text-gray-400"><p class="text-sm">Belum ada jadwal</p></div>
            <div v-for="s in schedules" :key="s.id" class="card mb-3">
                <div class="flex items-start justify-between mb-3">
                    <h3 class="font-semibold text-gray-900 flex-1 pr-3">{{ s.title }}</h3>
                    <button @click="toggleOpen(s)"
                ...
                <button @click="openValidation(s)"
                    class="mt-3 w-full text-center text-xs font-semibold text-brand-purple border border-brand-purple/30 rounded-xl py-2 hover:bg-brand-purple/5 transition-all active:scale-95"
                >
                    👥 Validasi Presensi
                </button>
            </div>
                ...
                <div class="px-6 pt-2 pb-10">
                    ...
                    <div v-if="!selectedPresence">
                        ...
                        <div v-else class="space-y-3">
                            <div v-for="p in validationPresences" :key="p.id"
                                ...
                                <img :src="p.user.profile_picture || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(p.user.name) + '&background=5B2163&color=fff&size=64'" class="w-12 h-12 rounded-full object-cover flex-shrink-0" />
                                ...
                                <span :class="['text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0', statusBadge[p.status]?.cls]">{{ statusBadge[p.status]?.label }}</span>
                    ...
                    <div v-else class="flex flex-col h-full max-h-[75vh]">
                        ...
                        <div class="flex items-center gap-3 pb-4 border-b border-gray-100 flex-shrink-0">
                            ...
                            <img :src="selectedPresence.user.profile_picture || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(selectedPresence.user.name) + '&background=5B2163&color=fff&size=64'" class="w-10 h-10 rounded-full object-cover flex-shrink-0" />
                            <div class="flex-1 min-w-0">
                            ...
                            <span :class="['text-xs font-bold px-2.5 py-1 rounded-md flex-shrink-0', statusBadge[selectedPresence.status]?.cls]">{{ statusBadge[selectedPresence.status]?.label }}</span>
                        </div>
                        ...
                        <div v-if="selectedPresence.status === 'menunggu'" class="pt-4 border-t border-gray-100 flex gap-3 mt-2 flex-shrink-0">
                            <button @click="setPresenceStatus(selectedPresence, 'hadir')" class="flex-1 flex flex-col items-center justify-center gap-1 py-2.5 rounded-xl bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 transition-all active:scale-95 shadow-sm">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                <span class="text-xs font-bold">Hadir</span>
                            </button>
                            <button @click="setPresenceStatus(selectedPresence, 'terlambat')" class="flex-1 flex flex-col items-center justify-center gap-1 py-2.5 rounded-xl bg-yellow-50 text-yellow-700 border border-yellow-200 hover:bg-yellow-100 transition-all active:scale-95 shadow-sm">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <span class="text-xs font-bold">Terlambat</span>
                            </button>
                            <button @click="setPresenceStatus(selectedPresence, 'tidak_hadir')" class="flex-1 flex flex-col items-center justify-center gap-1 py-2.5 rounded-xl bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 transition-all active:scale-95 shadow-sm">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                ...
                <div class="px-6 pt-2 pb-10">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center mt-2 mb-2">
                            <input v-model="eventForm.is_recurring" type="checkbox" id="is_recurring_pelatih" class="w-4 h-4 text-brand-purple border-gray-300 rounded focus:ring-brand-purple">
                            <label for="is_recurring_pelatih" class="ml-2 text-sm font-medium text-gray-700">Jadikan Kegiatan Rutin Mingguan</label>
                        </div>
