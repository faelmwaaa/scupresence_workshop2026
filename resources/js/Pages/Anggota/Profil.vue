...
const onFileChange = (e) => {
    ...
    // Upload via router with FormData
    const fd = new FormData();
    fd.append('profile_picture', file);
    router.post(route('profile.picture'), fd, {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    ...
    <div class="page-container bg-gray-50 pb-24">
        <div class="bg-gradient-to-b from-brand-purple to-purple-700 px-4 pt-14 pb-20 text-white text-center">
            <div class="relative inline-block">
                <img
                    :src="preview || user.profile_picture || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user.name) + '&background=fff&color=5B2163&size=128'"
            ...
            <p class="text-purple-200 text-sm">{{ user.nim }}</p>
        </div>

        <div class="px-4 -mt-8 space-y-4">
            <div class="card">
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide mb-3">Informasi Akun Google</p>
            ...
            <div class="card space-y-3">
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">Data Pribadi</p>
                <div class="grid grid-cols-2 gap-3">
                    <div><p class="text-xs text-gray-400">NIM</p><p class="text-sm font-medium text-gray-800">{{ user.nim || '-' }}</p></div>
                    <div><p class="text-xs text-gray-400">No. HP</p><p class="text-sm font-medium text-gray-800">{{ user.phone || '-' }}</p></div>
                    <div class="col-span-2"><p class="text-xs text-gray-400">Fakultas</p><p class="text-sm font-medium text-gray-800">{{ user.fakultas || '-' }}</p></div>
                </div>
            </div>
